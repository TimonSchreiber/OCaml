...
let is_palindrom (lst : 'a list) : bool =
  lst = rev lst
;;
...
let replicate (lst : 'list) (n : int) : 'a list =
  let rec put_first (acc : 'a list) (element : 'a) (n : int) : 'a list =
    match n with
    | 0 -> acc
    | _ -> put_first (element :: acc) element (n - 1)
  in
  let rec aux (lst : 'list) (acc : 'a list) =
    match lst with
    | [] -> acc
    | hd :: tl -> aux tl (put_first acc hd n)
  in
  rev (aux lst [])
;;
...
let permutation = function
  | [] -> []
  | lst -> rand_select lst (length lst)
;;
...
let phi m =
  let rec count_coprime r acc =
    if r < m then
      count_coprime (r + 1) (if coprime r m then acc + 1 else acc)
    else acc
  in
  count_coprime 2 1
;;
...
(* Problem 35: Compare the Two Methods of Calculating Euler' Totient Function (Beginner) *)
...
let all_primes a b =
  let rec aux n acc =
    if n <= b then
      aux (n + 1) (if is_prime n then n :: acc else acc)
    else
      acc
  in
  rev (aux a [])
...
let goldbach m =
  if m > 2 && m mod 2 = 0 then
    let rec aux d =
      if is_prime d && is_prime (m - d) then
        (d, m - d)
      else
        aux (d + 1)
    in
...
(* Problem 42: Huffman Code (Advanced) *)
...
let sum (Leaf (a)) (Leaf (b)) = a + b;;

let insert e root =
  let i = root root in
  match e with
  | (a, b) when a > i -> Node (b+i, (Leaf(e), root))
  | (_, b) ->  Node (b+i, (root, Leaf(e)))
;;

let huffman (lst: (string*int) list) =
  let lst = (List.sort (fun (_, f1) (_,f2) -> if f1 < f2 then -1 else if f1 > f2 then 1 else 0) lst) in
  let tree ((a, fa) :: (b, fb) :: _) = Node (fa+fb, (Leaf (a,fb), Leaf (b,fb))) in
  let tail (_ :: _ :: tl) = tl in
  let rec aux (acc: ('a * int) tree) = function
    | [] -> acc
    | hd :: tl -> aux (insert hd acc) tl
  in
  aux (tree lst) (tail lst)
;;
