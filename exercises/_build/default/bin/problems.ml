...
let pack (lst: int list): int list list =
  let rec aux (lst: int list) (current: int list) (acc: int list list): int list list =
    match lst with
    | [] -> [] (* ? *)
    | [x] -> (x :: current) :: acc
...
let encode (lst: 'a list): (int * 'a) list =
  let rec aux (lst: 'a list) (count: int) (acc: (int * 'a) list): (int * 'a) list =
    match lst with
    | [] -> acc
    | [x] -> (count + 1, x) :: acc
...
let slice (lst: 'a list) (i: int) (j: int): 'a list =
  let rec aux (lst: 'a list) (acc: 'a list) (min: int) (max: int): 'a list =
    match lst with
    | [] -> acc
    | hd :: tl when min <= 0 && max >= 0 -> aux tl (hd :: acc) (min - 1) (max - 1)
...
let power a b =
  let rec aux a b res =
    if b < 1 then res else aux a (b-1) (res*a)
  in
  aux a b 1
;;
